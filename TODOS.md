...
- [ ] fallback סמנטי שווה לכל חוויית particles — האינטרו כן (הוא נסתר לגמרי
      ללא JS והעמוד העריכתי מתחתיו נשאר שמיש), אבל אין audit שמוכיח שוויון בשאר.
...
- [ ] **שתי לולאות render ממשיכות לרוץ בלשונית מוסתרת.** `visibilitychange`
      ...
      ([`Scene.tsx`](components/intro-scene/Scene.tsx)); קנבס הצ׳ט לא
...
- [ ] אין מדידת FPS או זיכרון בבית — `onFrameStats` קיים אך אין לו צרכן:
      ה־harness שהציג אותו (`/particle-demo`) נמחק עם הניווט הרדיאלי ב־1.9.2026.
...
- [ ] visual regression — אין baseline, אין compare, אין diff, ואין שלב צילום
      ב־CI. הסקריפט היחיד שצילם (`verify-composition.mjs`) נמחק עם הניווט
      הרדיאלי ב־1.9.2026, כך שהיום גם אין ממה להתחיל.
- [ ] כיסוי שגיאות קונסול ב־CI מדלג על `/admin` ו־`/admin/login`.
...
- [ ] **טקסט האינטרו מעולם לא נבדק מרונדר ב־320×568** — האילוצים מכוסים אנליטית
      בשתי בדיקות יחידה, אבל `capture.mjs` מצלם 390×844 ולא 320×568. שתי
      הבדיקות גם מדלגות על השורה האחרונה ומכסות ציר אופקי בלבד.
...
- [ ] שירות visual regression מנוהל — אין היום חלופה קודית: הסקריפט שצילם
      נמחק ב־1.9.2026, כך שהחלופה המקומית דורשת קודם לכתוב צלם חדש.
...
| 25.8.2026 | ייצוב P0 — layout רספונסיבי לאינטרו, תקרת 86vw, בעלים אחד למתמטיקת ה־viewport | ↑ |
...
- **אין יעד תשיעי ב־`SITE_NAVIGATION`** — הארכיונים ועמודי המחקר הם נתיבי ילד של
