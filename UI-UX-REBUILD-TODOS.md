...
- [ ] **ADMIN-001 — High** — Resolve the authorized local auth configuration before visual browser work on `/admin`; never commit the secret. **Acceptance:** route renders through normal auth flow locally. Depends on: operations.
  - Update 2026-09-03 (later session): `.env.local` `NEON_AUTH_COOKIE_SECRET` replaced with a freshly generated 64-hex-char value; the file is gitignored and the secret is not committed. Mark only after `/admin` is confirmed to render locally through the normal auth flow (requires a dev-server restart to pick up the new env).
