...
- **Six task groups have real, substantial, typechecked-and-linted work committed against them, but are held open because no acceptance criterion was independently confirmed** — see the dated 2026-09-03 sub-bullets directly under SEARCH-002, OCT-001 through OCT-007, PERF-004, ADMIN-002, STATE-004 (admin half), AUTH-001 (`/admin/login` half), and PIPE-001 through PIPE-004 for the exact evidence and the exact gap in each case. In one word each: SEARCH-002's backend work exists but is deliberately not merged into `main` yet, pending a migration-numbering reconciliation the owner is handling separately, and its row UI was never built regardless; the OCT/PERF-004 group is missing a verified per-task report (the agent was killed before sending one, so committed volume is not being taken as a pass); PIPE-001/002 are missing an actual browser viewport pass (the single worst-measured defect in the original audit); PIPE-003/004 and the admin group are missing only a runtime/browser confirmation of code that reads as complete.
