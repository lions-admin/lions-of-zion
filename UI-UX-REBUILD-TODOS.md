...
- [ ] **SEARCH-002 — Critical** — Rebuild result rows to expose entity type, verification state, date, excerpt, and destination. **Acceptance:** results are distinguishable without color and long text wraps at 320px. Depends on: SYS-008, SYS-011.
  - Blocked again 2026-09-03 (later session), differently than before: the backend half is now built — `server/contracts/search.ts` carries `date`, `verdict` (derived, not model-set; null where the indexed kind has no verdict, and rows predating migration `0049_search_result_substance` read as absent, never as unverified) and a query-cut excerpt; `server/modules/search/projection.ts` and `repo.ts` populate them; `tests/search-projection.test.ts` pins the shapes. **`components/search/SearchResults.tsx` and `search.module.css` were never touched** — confirmed by diff, not assumption — so the row still renders only entity type and title. The agent was killed by a run of API outages (HTTP 500, then repeated 529 Overloaded) before reaching the row UI, including on solo retries with no other agent competing for capacity. Do not mark until the row exposes date/verdict/excerpt/destination per the acceptance.
...
- [ ] **PIPE-002 — Critical** — Add one semantic `h1`, landmarks, logical keyboard sequence, 44px coarse-pointer controls, and visible focus. **Acceptance:** current 27–30px controls are gone and all 88 controls remain reachable. Depends on: PIPE-001.
  - Same status as PIPE-001 above — not independently verified.
...
## 20. Session status and interruption log — 2026-09-03

- [ ] **Section complete** — Not applicable; this is a status record, not a task list.

This section is a factual record of one implementation session, kept separate from the task list above so a status snapshot never gets mistaken for a task. It answers two things precisely: what is genuinely missing right now, and what caused the gaps.

### What is precisely missing right now

80 of 142 numbered tasks are checked complete with dated verification evidence in their own sub-bullets. The 62 that remain open break down as:

- **Six task groups have real, substantial, typechecked-and-linted work committed against them, but are held open because no acceptance criterion was independently confirmed** — see the dated 2026-09-03 sub-bullets directly under SEARCH-002, OCT-001 through OCT-007, PERF-004, ADMIN-002, STATE-004 (admin half), AUTH-001 (`/admin/login` half), and PIPE-001 through PIPE-004 for the exact evidence and the exact gap in each case. In one word each: SEARCH-002 is missing its row UI entirely (backend done, `SearchResults.tsx` untouched); the OCT/PERF-004 group is missing a verified per-task report (the agent was killed before sending one, so committed volume is not being taken as a pass); PIPE-001/002 are missing an actual browser viewport pass (the single worst-measured defect in the original audit); PIPE-003/004 and the admin group are missing only a runtime/browser confirmation of code that reads as complete.
- **The remaining ~46 open tasks were never started this session**: QA-001 through QA-011, A11Y-001 through A11Y-010, PERF-001/002/003/005/006/007/008/009, RESP-001 through RESP-005, CLEAN-002/003/004/007/008, MOTION-002/003/004, STATE-003/005, SYS-015, and LIVE-004. These are almost entirely cross-cutting passes over the *finished* route work (geometry checks at nine viewports, keyboard journeys, contrast, bundle/perf budgets, dead-code removal) rather than route rebuilds, and per the document's own **Execution order** (§17) they were always meant to run after route migration, not during it.

### Interruption log

Across roughly the second half of this session, background implementation agents were repeatedly killed by transient infrastructure errors unrelated to their own work — never by a code defect, never by a permission refusal:

1. **Session/rate limit (HTTP 429).** Three agents (October 7 archive, institution pages, support/account) were killed mid-task by a session limit reset boundary. All three were resumed from their own worktree state; none had lost committed work, and the two that had uncommitted work at that point (October 7, support/account) still had it sitting untouched in their worktrees when resumed.
2. **First `git stash` collision.** Independently of the outages, two agents (institution pages, support/account) each ran `git stash` to test something, nine seconds apart, in this repository's *shared* stash stack (`git stash` is not scoped per worktree). The second agent's `git stash pop` pulled the first agent's changes into its own worktree. Both sets were fully recovered and verified byte-identical against what each agent went on to commit; nothing was lost. Every agent since has been explicitly instructed never to use `git stash`, `git stash pop`, `git stash apply`, or `git stash drop` in this repository.
3. **Server errors (HTTP 500), four agents simultaneously.** October 7 archive, admin, pipeline, and search agents were killed together with consecutive API request IDs — a server-side incident, confirmed by the identical timing and error shape across unrelated worktrees.
4. **Overload errors (HTTP 529 "Overloaded"), repeated.** The same four agents failed again immediately on resume, then again after a ten-minute backoff, then again on a further attempt running only one or two agents at a time rather than four. Capacity did not visibly recover for the remainder of the session. Every uncommitted file in every affected worktree was protected by a parent-session safety commit (clearly labelled `wip(...)`, not claimed as agent-verified work) before any further resume attempt, so no outage after the first cost any file.

### What changed as a direct result

- After incident 1, every subsequent agent brief added an explicit "commit early and often, even mid-thought" instruction; this is why incidents 3 and 4 cost zero files despite killing agents repeatedly.
- After incident 2, every subsequent agent brief added an explicit, unconditional ban on `git stash` in this repository.
- After incident 4 made further agent dispatch unproductive, the parent session did the remaining reachable work directly instead of continuing to retry: it merged all four in-flight worktree branches into an integration branch, found and fixed a real compile break the pipeline agent left mid-rename (`NodeInspectorDrawer` renamed to `NodeInspector` without updating its one caller), then discovered and closed the gap behind PIPE-004 — `StructureListView` was fully written but mounted nowhere and shipped with none of its 40 CSS classes defined, confirmed by grep before writing the fix, not assumed.
- The owner's own uncommitted work in the primary checkout (33 files, a briefing/codex-import feature, and a colliding `0049` migration number) was identified early and left completely untouched throughout; all integration work after that point happened in a separate git worktree specifically to guarantee that.
