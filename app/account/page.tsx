...
/* `pageMetadata`, like every other page on the site: it carries the canonical
   URL, the OG and Twitter cards and the title template this route was writing
   a bare two-field object instead of. */
 ...
 * `<main>`, no footer, no banner/contentinfo landmarks — and a glass card with
 * a 16px backdrop blur, which is not a thing this design system has. It now
 * wears `EditorialShell` like every other institution page, so the chrome, the
 * landmarks, the family density and the way out are the same ones the rest of
 * the site uses, and the page itself is one plain panel on the navy ground.
 *
