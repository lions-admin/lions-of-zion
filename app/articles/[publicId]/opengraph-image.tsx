...
export default async function Image({ params }: { params: Promise<{ publicId: string }> }) {
  ...
  return new ImageResponse(
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 24, letterSpacing: 5 }}>
        <span>LIONSOFZION</span><span style={{ color: "#d2a94f" }}>{article.section.replaceAll("_", " ").toUpperCase()}</span>
      </div>
