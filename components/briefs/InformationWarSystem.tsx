  ...
  return (
    <EditorialShell routeId="information-war" register="silent" className={styles.page} progressTrackClassName={styles.progressTrack}>
      <section className={styles.hero} id="page-content" aria-labelledby="war-heading">
        ...
        <div className={styles.heroGrid}>
          ...
          <div className={styles.problem} id="problem">
            ...
            <p>The information environment can turn one claim into a wall of apparent confirmation. Lions of Zion uses technology to trace origin, source family, context, propagation and uncertainty — then makes those distinctions visible to the reader.</p>
          </div>
        </div>
      ...
      <section className={styles.section} id="system" aria-labelledby="system-heading">
        <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>01 / The architecture</p><h2 id="system-heading">Machine scale.<br /><em>Visible provenance.</em></h2></div><p>AI can discover, compare, organize, draft and operate authorized editorial workflows. Different records take different paths, but none is allowed to blur source, evidence, authorship and assessment into one opaque output.</p></div>
        <PipelineTrace />
      </section>

      <section className={`${styles.section} ${styles.accountability}`} id="cycle" aria-labelledby="cycle-heading">
        <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>02 / The standard</p><h2 id="cycle-heading">AI-powered.<br /><em>Human-governed.</em></h2></div><p>People define the mission, source standards, publishing permissions, provenance rules, escalation paths, corrections policy and safety boundaries. AI expands reach and speed; it does not become evidence and it does not eliminate accountability.</p></div>
        <div className={styles.evidenceLedger}>
          <div><span className={styles.eyebrow}>Keep the distinctions</span><h3>Claim.<br />Evidence.<br /><em>Assessment.</em></h3><p>Different things.<br />Never interchangeable.</p></div>
          <ol>
        ...
        <section className={styles.section} id="walkthrough" aria-labelledby="walkthrough-heading">
          <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>A worked example</p><h2 id="walkthrough-heading">Put the narrative<br /><em>to the test.</em></h2></div><p>Follow a circulating claim through source discovery, comparison and assessment. Examine both supporting and contradicting material, and keep propagation separate from proof.</p></div>
          <HomeEvidencePipeline />
        </section>
      </section>

      <section className={styles.section} id="record" aria-labelledby="record-heading">
        <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>03 / In public</p><h2 id="record-heading">The work,<br /><em>in the open.</em></h2></div><p>Reporting to read. Evidence to inspect. Records to return to. Machine-authored work is disclosed, archive material keeps its own provenance, and readers can follow the sources behind consequential claims.</p></div>
        <OutputsFork />
      ...
      <section className={styles.support} aria-labelledby="support-heading">
        ...
        <div><p>Support the work behind the page: source research, documentation, public reporting and the technology that makes information manipulation easier to inspect.</p><Link href="/support-us" className={styles.supportLink}>Support Lions of Zion <span aria-hidden="true">↗︎</span></Link><Link href="/methodology" className={styles.methodologyLink}>Read the methodology</Link></div>
      </section>
    </EditorialShell>
  );
}
