...
export function HomeNarrativesSection({
  ...
  return (
    <section
      id="home-narratives"
      className={`${styles.section} ${styles.investigations}`}
    ...
    >
      <SectionHeading
        ...
        action="Explore the investigations"
      />
                    ...
                    <ResearchText>
                      {item.finding ??
                        ...
                        item.verification === "unresolved"
                          ? "No finding has been reached. Monitoring is not confirmation."
                          : "Read the record for the assessment and its limitations; no specific finding excerpt is selected here.")}
                    </ResearchText>
                  </p>
                </div>
                <p className={styles.sources}>
                  {item.kind === "case"
                    ? `${item.sourceCount} sources in the case; source count is not a verdict.`
                    : item.basis === "analysis"
                      ? "Lions of Zion editorial analysis · No source-backed finding is implied."
                      : item.sources.length
                        ? "Source-backed monitoring record."
