...
export function HomeNarrativesSection({section}:{section:HomepageEdition['fakeResistance']}){
 return <section id="home-narratives" className={`${styles.section} ${styles.investigations}`} aria-labelledby="home-narratives-title" data-home-section="fakeResistance">
 ...
 <div className={styles.finding}><span>{item.kind==='watch'?'Finding':'From the research'}</span><p><ResearchText>{item.finding??(item.kind==='watch'&&item.verification==='unresolved'?'No finding has been reached. Monitoring is not confirmation.':'Read the record for the assessment and its limitations; no specific finding excerpt is selected here.')}</ResearchText></p></div>
