...
export function HomeSystemSection(){return <section id="home-system" className={`${styles.section} ${styles.system}`} aria-labelledby="home-system-title" data-home-section="system">
 <div className={styles.systemOverview}><header className={styles.systemHead}><p className={styles.kicker}>Behind every record</p><h2 id="home-system-title">From a source.<br/>To something you can examine.</h2><p>This is the architecture behind the desk—not a live activity monitor. Sources, claims, assessments and published records remain distinct.</p></header>
 <ol className={styles.pipeline}>{steps.map(([name,description])=><li key={name}><strong>{name}</strong><span>{description}</span></li>)}</ol></div>
