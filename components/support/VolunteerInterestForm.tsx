...
export function VolunteerInterestForm() {
  ...
  /* The receipt's live region is mounted in every state, so a screen reader
     has a region to speak from when the send lands — a region inserted in the
     same commit as its message is often announced as nothing at all. Same
     shape as the report form's receipt (2026-09-17 support round): what
     happens next, the expected window, the desk's address, and the two ways
     onward. */
  return (
    <>
      {status === 'sent' ? null : (
        /*
          A11Y-007. The send failure is a fact about the submission, not about
          any one field — the endpoint composes a mail from all five and reports
          one result — so it stays a form-level summary and the form is described
          by it. The no-JavaScript notice is referenced the same way: with
          scripting on it is not in the DOM at all and the reference is simply
          ignored, which is the behaviour wanted in both tiers.
        */
        <form
      ...
      <div className={status === 'sent' ? styles.receipt : undefined} {...politeLive}>
        {status === 'sent' ? (
          <>
            <p>Thanks — your interest reached the volunteer desk.</p>
            <small>
              What happens next: the volunteer desk reads every note and replies
              by email when there is a role that fits what you offered. Most
              are read within a few days.
            </small>
            <small>
              To reach the desk another way, email{' '}
              <a href={`mailto:${VOLUNTEER_INBOX}`}>{VOLUNTEER_INBOX}</a>.
            </small>
