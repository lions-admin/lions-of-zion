...
/**
 * Shared status grammar for evidence, verification, and system chrome — the
 * one verdict grammar on the site (2026-09-16): `VerificationBadge`,
 * `EvidenceGrade` and the roster's identity chip all render through here.
 *
 * The mark is a function of verdict *polarity*, not of status: a filled disc
 * affirms, a square negates, a diamond is contested, a dashed disc marks
 * altered material, a hollow disc is unassessed. Two statuses in the same
 * colour ramp — "Verified" (ok) and "Success" (ok), "False" and "Error" —
 * still differ once the colour is gone, which is what the mark is for.
 */
...
export const BADGE_GRAMMAR: Record<BadgeStatus, BadgeGrammar> = {
  ...
  ember: { ramp: "ember", label: "Contested", mark: "diamond", domain: "verification" },
  ok: { ramp: "ok", label: "OK", mark: "circle", domain: "system" },
  warn: { ramp: "warn", label: "Warning", mark: "diamond", domain: "system" },
  danger: { ramp: "danger", label: "Alert", mark: "square", domain: "system" },
  verified: { ramp: "ok", label: "Verified", mark: "circle", domain: "verification" },
  ...
  /* Verification polarity: affirming = circle, negating = square,
     uncertain = diamond, altered = dashed, unassessed = hollow. */
  ...
  manipulated: { ramp: "danger", label: "Manipulated", mark: "dashed", domain: "verification" },
  out_of_context: { ramp: "ember", label: "Out of context", mark: "dashed", domain: "verification" },
  ...
  /* Evidence: what kind of proof stands behind an edge, and how well the
     research knows it. A documented/observed/high proof affirms; an inferred
     or low one is held at arm's length. */
  documented: { ramp: "ok", label: "Documented", mark: "circle", domain: "evidence" },
  ...
  high: { ramp: "ok", label: "High confidence", mark: "circle", domain: "evidence" },
