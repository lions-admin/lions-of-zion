...
| Control | tasks | Operational task board: every reporting environment's tasks with status, timeline, narrative, links and attachments; keyset paging, per-task drawer, manual status marks with a required note, the reporting-coverage roster (connected only on a recorded `lastSeenAt`), and the hooks-and-automations inventory (`hooksInventory`: the newest machine-collected list of every AI tool's hooks on the reporting machine — one card per tool with active/partial/none status, config path and a hook table of event, command, source and timeout; a single absence line until the first Claude or Grok session sends one). Narrative fields render in full with line breaks and `- ` bullet lists; a timeline message over 600 characters sits behind a show-all toggle |
