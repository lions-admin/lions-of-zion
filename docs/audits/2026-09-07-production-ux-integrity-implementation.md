...
| VA-51 | A6 | ☑ done | 4a | `05e6dd8` — /information-war unified, seven breadcrumbs derived, 404 desk fixed. `aa7a68f` — 51.1 job-of-each-destination table (eleven destinations) written into §6; `/ask`'s "AI Chat" fourth/fifth name replaced with the chrome's "Ask the desk" in `AskDock.tsx`, test updated |
...
| VA-58 | A6 | ☐ not started | 4a | `aa7a68f` narrows this: the Ask launcher's naming (58.3, the "AI Chat" name) is already fixed as a side effect of closing 51.1 — see that step's evidence. What's still open: 58.2 (state the Search/Ask distinction in product terms beyond naming), 58.4 (Ask must not look like a second search box — `SearchDialog`'s own copy was not touched here) |
...
(`5757712`), taking the Ask fix out with everything else. `aa7a68f` redoes just
that one piece while closing 51.1 — worth knowing so nobody reads the file
history at `2c40e63` and assumes the fix is already live.
