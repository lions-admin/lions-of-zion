...
| 6 | **October 7** (`#home-archive`, "The record remains.") | October 7 | `/october-7/…`; "All of October 7" `/october-7` | Static archive catalogue (`lib/content/testimonies.ts`, `documentation.ts`) | One testimony + one documented record, each behind its warning | `october7.testimony` / `october7.documentation` featured slots (`server/modules/featured-slots`, 2026-09-14): 2–4 day dwell against a candidate pool of every eligible testimony and documented record. **Not placeable** via `homepage`, but a run may `pin`/`release` either slot through the package's `featured` block | **None**, deliberately: the record's date is an ingestion date, not the event's | Development / archive curation. A run may pin or release a slot; it never creates archive material |
...
| 7a | └ **Courage & service** sub-block | The People of Israel | `/our-heroes#…` | Static Our Heroes edition (`lib/content/our-heroes.ts`), role Rescuer or Fighter | One profile | `heroes.courage` featured slot: 2–4 day dwell, excludes whichever profile `heroes.fallen` holds this pass | **None**: profile dates are ingestion dates | Development; a run may pin or release the slot |
| 7a′ | └ **Fallen** sub-block (rendered separately from Courage & service since 2026-09-14) | The People of Israel | `/our-heroes#…` | Static Our Heroes edition (`lib/content/our-heroes.ts`), role Fallen | One profile | `heroes.fallen` featured slot: same rule as 7a | **None** | Development; a run may pin or release the slot |
| 7b | └ **History & context** sub-block ("Beyond the current headline") | The People of Israel | `/israels-story#…` | Static Israel's Story chapters (`lib/content/israels-story.ts`) | Two chapter links with an era label; "Contested" where the chapter records disagreement | `history.primary` / `history.secondary` featured slots: 2–4 day dwell each, `secondary` excludes whichever chapter `primary` holds this pass | **Era label** only (e.g. `1947`), never a record date | Development; a run may pin or release a slot |
...
| **Evergreen, rotated** | October 7, Courage & service, Fallen, History & context | Each of the six featured slots rotates on a 2–4 day dwell (`server/modules/featured-slots`), independent of the edition cycle; a run may pin or release a slot but never creates archive/hero/chapter material |
