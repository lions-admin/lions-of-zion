...
- [x] Pass an intro opacity multiplier into `components/particle-nav/layers/NetworkScan.tsx` and multiply, rather than replace, `scanFieldOpacity`, `scanWordOpacity`, and `scanGlyphOpacity` from `defaultSimParams`. *(`experienceFrameRef` prop; `introScanMultiplier(scanReveal, navReveal, target)` in `components/intro/scanIntro.ts` = `mix(target·scanReveal, 1, navReveal)`; pinned by `tests/intro-scan.test.ts`.)* **2026-09-04:** the layer's `useFrame` now subscribes at `SCAN_FRAME_PRIORITY` 0.5 — above `Scene`'s default-priority writer and below the post chain's 1 — so it reads the frame `Scene` just wrote instead of the previous one. r3f sorts `internal.subscribers` by priority and a child subscribes first, so a default-priority read here was a frame late on the hero hole and the corridor during the fastest part of the rise. No change to `Scene.tsx` was needed: `internal.priority` only gates r3f's auto-render, which the priority-1 post pass had already switched off.
- [x] In `components/particle-nav/tsl/networkScanMaterial.ts`, make the hero exclusion mask capable of following the relocated lion and add a separate soft text-column exclusion/muting region. Prefer uniforms so the masks move without rebuilding geometry/materials. *(Uniforms `heroCenterY`/`heroMaskX`/`heroMaskY` from `lionY`/`lionScale`, and `corridorY`/`corridorHalfHeight`/`corridorHalfWidth`/`corridorStrength` from the live story rows and `readingMask`; the corridor dims to 15% (`TEXT_CORRIDOR_MUTE` 0.85), never hard-cuts; static node holes kept.)* **Completed 2026-09-04:** the first pass left `createExclusionTest` in `NetworkScan.tsx` still punching a centred ellipse (`1.34`/`1.18 × orbit.centerScale`) out of the field *geometry* at build time, so an empty oval stayed at world centre for the whole intro no matter what the uniforms said. That ellipse is gone; only the eight spoke exclusions remain build-time. The uniform mask's fully dark core is 3.97% wider in x and 3.49% wider in y than the retired hole at every `centerScale`, so the navigation state is unchanged — `tests/intro-scan.test.ts` proves it against `lionScanMaskOpacity`, the CPU mirror that shares `LION_MASK_EDGE_INNER`/`OUTER` with the shader.
...
- [~] Lion silhouette and every text line retain a clean contrast corridor. *(Code: hero hole follows `lionY`/`lionScale`; `solveScanCorridor` is tested to cover every visible row and the brand at every cue on both layouts. Browser check pending — but the one thing a capture would have shown is fixed: the build-time centred hole in the field geometry, which put a static empty ellipse under the story column, was removed on 2026-09-04. It covered 6.7% of the field at 1440×900 and 17.8% at 390×844. Point count is unchanged at `tier.networkPoints`; the row budget is now an even share of the remainder rather than a fixed `pointBudget / rows`, which holds the top band's density change to 1.9% instead of 10.6% at 1440×900 / 18k. Narrow viewports still spread the same budget over 14–18% more area — irreducible without re-freezing a hole, and the centre it fills is masked in navigation and muted by the text corridor during the story.)*
