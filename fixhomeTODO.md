...
- [ ] Keep the narrative available to assistive technology through the existing semantic `article` in `CanvasMount.tsx`. **Deferred — belongs to the intro-canvas owner, not the site layer.** Flagged from here, because it is worse than an unticked box: `CinematicIntroGate` initialises `blocked = true`, so the server HTML renders the entire home inside `<div class="introDestination" inert aria-hidden="true">`, and only hydration clears it. With JavaScript off, nothing ever does — every link on the front page is unfocusable and the whole document is out of the accessibility tree. The `<noscript>` blocks in `CinematicIntroGate.tsx` and `app/page.tsx` are CSS, and neither `inert` nor `aria-hidden` is reachable from CSS, so this cannot be fixed from `app/home.module.css` or from the page's `<noscript>`. It has to be fixed where the state is: render those two attributes only after mount, or gate them behind something a `<noscript>` rule can reach.
...
- [ ] Skip Intro retains a visible `:focus-visible` style and safe-area spacing. (`components/particle-nav/**` — another agent's surface.)
- [ ] The intro never traps keyboard focus; Escape completes it. (`components/particle-nav/**`.)
- [ ] After handoff, the first user gesture cannot be retargeted to a newly revealed link on iOS/WebKit. (`components/particle-nav/**`.)
