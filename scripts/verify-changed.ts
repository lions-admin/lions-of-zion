...
const VISUAL_ASSET = /^(?:assets\/(?:reference|source)\/|public\/(?:assets|particles|posters)\/)/;
const UI_CODE = /^(?:app|components)\/.*\.(?:tsx|css)$/;
const INTRO = /^(?:components\/intro\/|components\/intro-scene\/(?:Scene\.tsx|layers\/IntroText\.tsx)|\.claude\/skills\/verify-intro\/)/;
const ARCHITECTURE_SENSITIVE = /^(?:AGENTS\.md|package(?:-lock)?\.json|eslint\.config\.mjs|tsconfig\.json|next\.config\.ts|vitest\.config\.ts|scripts\/project-map(?:-prose)?\.mjs|docs\/PROJECT_MAP\.md|docs\/project-map\.html|components\/intro-scene\/config\.ts|app\/sitemap\.ts)$/;
