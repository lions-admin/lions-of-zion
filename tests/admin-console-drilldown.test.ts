...
/* This file's mock is independent of the one in `admin-console-p2.test.ts`,
   so the boundary it installs needs its own proof. Same four claims, driven
   through this file's own route. */
...
describe("withDatabaseRole boundary", () => {
  it("establishes the role and the identity, and runs the callback inside them", async () => {
    ...
    const inside = await withDatabaseRole("app_staff", "admin:drilldown", async () => {
    ...
    expect(inside.identity).toBe("admin:drilldown");
    /* One transaction id across both statements: the callback ran inside a
       single continuous scope rather than around it. */
  ...
  it("puts the role's privileges in force — app_public may not read the audit log", async () => {
    ...
    /* Drizzle wraps the driver error as `Failed query: …`, so assert on the
       SQLSTATE rather than the wrapper's message. */
    const refused = await violation(
      withDatabaseRole("app_public", "anonymous:drilldown", async () => {
    ...
    /* The same read succeeds as app_staff, so the refusal is the role and not
       a broken query. */
    await expect(
      withDatabaseRole("app_staff", "admin:drilldown", async () => {
        await db.execute(sql`SELECT count(*) FROM audit_log`);
      }),
  ...
  it("propagates a failure out of the boundary and still releases the scope", async () => {
    ...
    await expect(
      withDatabaseRole("app_service", "service:drilldown-failure", async () => {
        throw boom;
      }),
    ).rejects.toBe(boom);

    /* The scope was entered before the throw — a wrapper that swallowed the
       callback would record nothing here. */
    expect(state.observed).toEqual([
      { role: "app_service", identity: "service:drilldown-failure" },
    ]);
    const after = await scopeOf(db);
    expect(after.role).toBe(after.owner);
    expect(after.identity ?? "").toBe("");
  ...
  it("runs the editions route as app_staff, after the app_service auth bootstrap", async () => {
    const db = await freshDatabase();
    state.db = db;
    ...
    /* The wrapper is invoked twice, with the roles `handler()` classifies, and
       each scope was really in force once inside it. */
    const expected = [
    ...
    /* databaseIdentity() is back to its default now every scope has closed. */
