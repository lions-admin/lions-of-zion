...
const { cases, watch } = vi.hoisted(() => ({ cases: vi.fn(), watch: vi.fn() }));
vi.mock("@/lib/content/fake-resistance-cases", () => ({ getCaseIndex: cases }));
vi.mock("@/lib/content/fake-resistance-watch", () => ({ getNarrativeWatchFeed: watch }));
...
beforeEach(() => { cases.mockReset(); watch.mockReset(); });
